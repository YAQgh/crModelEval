...
concord_ipw <- function( score_l, l, n_l, G_hat, types ){
  # fast computation algorithm has been implemented
  # score_l: CIF or S of cause l
  # l: cause of interest
  # n_l: number of outcome categories including event-free
  # G_hat: estimated survival function of censoring at observed time or tau
  # types: location of subjects in each categoty
  ns = rep(1, n_l)# record number of subjects in each category
  for( ll in 1:n_l ){
    tmp = score_l[ types[[ ll ]] ] # CIF or S of subjects in cause ll
    types[[ ll ]] = types[[ ll ]][ order( tmp, decreasing=TRUE ) ] # decreasing order, ordered by estimated CIF
    ns[ll] = length(types[[ ll ]])
  }
  inv_G_hat = 1 / G_hat
  ls_sum = rep(1, n_l) # sum of G inverse in each category. S1 S2 and S3 in paper Algorithm 1.
  for( ll in 1:n_l ){
    ls_sum[ll] = sum( inv_G_hat[ types[[ ll ]] ] )
  }
  idxs = rep(1, n_l); cnt = 0 # index for each category, i.e., i j k mentioned in the paper when there are three categories
  eq_sum = rep(0, n_l); choice_seq = rep(1, n_l)
  while( idxs[l] <= ns[l] ){ # Take l=1 for example. checking i<=n_1
    eq_sum[l] = inv_G_hat[ types[[ l ]][ idxs[l] ] ] # 1/G(X_i)
...
pdi <- function( tau, scores, x, delt_eps, G_hat, n_l ){
  ...
  for( ll in 1:(n_l - 1) ){
    types[[ ll ]] = which( x <= tau & delt_eps == ll) # location of subjects from each class
    ns[ll] = length( types[[ ll ]] ) # number of subjects in each class
    denom = denom * sum( 1 / G_hat[ types[[ ll ]] ] ) # denominator
  }
  types[[ n_l ]] = which( x > tau ) # location of survivor at tau
  ns[n_l] = length( types[[ n_l ]] ) # number of survivor at tau
